...
public class SecurityController implements ISecurityController {
    ...
    public @NotNull Handler addRole() {
        return (ctx) -> {
            ObjectNode returnObject = objectMapper.createObjectNode();
            try {
                var body = ctx.bodyAsClass(ObjectNode.class);
                if (body == null || !body.hasNonNull("role") || body.get("role").asText().isBlank()) {
                    ctx.status(HttpStatus.BAD_REQUEST).json(returnObject.put("msg", "Role is required"));
                    return;
                }

                String rawRole = body.get("role").asText();
                SecurityRole requestedRole;
                try {
                    requestedRole = SecurityRole.valueOf(rawRole.trim().toUpperCase(Locale.ROOT));
                } catch (IllegalArgumentException ex) {
                    ctx.status(HttpStatus.BAD_REQUEST).json(returnObject.put("msg", "Unknown role: " + rawRole));
                    return;
                }

                if (requestedRole == SecurityRole.ANYONE) {
                    ctx.status(HttpStatus.BAD_REQUEST).json(returnObject.put("msg", "Role ANYONE cannot be assigned"));
                    return;
                }

                UserDTO user = ctx.attribute("user");
                securityDAO.addRole(user, requestedRole.name().toLowerCase(Locale.ROOT));
                ctx.status(HttpStatus.OK).json(returnObject.put("msg", "Role " + requestedRole + " added to user"));
            } catch (IllegalArgumentException e) {
                ctx.status(HttpStatus.BAD_REQUEST).json(returnObject.put("msg", e.getMessage()));
